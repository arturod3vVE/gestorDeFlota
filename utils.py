...
def get_cookie_manager():
    # Usamos una key constante para que no se reinicie el componente
...
def verificar_login():
    ...
    if cookie_user and not st.session_state.autenticado:
        ...
        # TRUCO PARA PWA: Renovar la cookie cada vez que entra automáticamente.
        # Esto le dice al navegador "Oye, esta cookie sigue siendo importante, dame 30 días más".
        try:
            cookie_manager.set("gestor_flota_user", cookie_user, expires_at=datetime.now() + timedelta(days=30))
        except:
            pass # Si falla la renovación visual, no importa, ya estamos dentro
        ...
        with tab_login:
            ...
            if btn_in:
                if validar_usuario_db(user, pwd):
                    ...
                    if mantener:
                        # Aseguramos 30 días de persistencia
